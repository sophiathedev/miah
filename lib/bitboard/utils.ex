...
defmodule Bitboard.Utils do
  @compile {
    ...
    ls1b: 1, popcount: 1, pop_lsb: 1, bit_square: 1, next_bit: 1, bit_rank: 1, bit_not_rank: 1
  }
  ...
  def shift(b, direction) do
    # and operation with full board mask for guarantee that after shift the bit already on the board
    case direction do
      :north -> b <<< 8
      :south -> b >>> 8
      :north_north -> b <<< 16
  ...
  def all_shift(x) do
    ...
    all_shift =
      all_direction |> Stream.map(fn dir -> shift(x, dir) end) |> Enum.reduce(0, &bor(&1, &2))
